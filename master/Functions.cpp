...
int checkArguments(int argc, char **argv, int &workerNum, int &bufferSize, int &serverPort, char *&serverIP, string &path) {
    bool arS = false;
    bool arP = false;
    bool arI = false;
    bool arW = false;
    ...
    if(argc != 11) {
        ...
        for(int i = 1; i < argc; i += 2 ) {
            ...
            else if((strcmp("-s", argv[i]) == 0) && !arS) {
                if(i + 1 < 9){
                    serverIP = new char[strlen(argv[i+1])];
...
//        else summary = summary + file_array[z]->date + "?";
        while (getline(&line, &lenght, fp) != -1) {
            if(file_array[z]->date.c_str() == NULL) {
                cout << "date is null" << endl;
                continue;
            }
            if(countryS == NULL) {
                cout << "countryS is null" << endl;
                continue;
            }
            record = new Record();
...
//    cout << "summary: " << summary << " for " << countryS << endl;
    if(signals != SIGUSR1) {
        fflush(stdout);
